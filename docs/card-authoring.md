...
### 宣言型プレイの一覧と、同時に宣言できること

カード直下に書く「手札からの宣言型プレイ」は現在この5つで、**すべて同じ枠組み**を通ります。

| kind | 契機 | 既定の `target` | コスト |
|---|---|---|---|
| `oniEnd` | 指定（既定 `attack`） | `both` | 支払わない |
| `ddd` | 指定（既定 `attack`） | `both` | **支払う**（`cost`） |
| `attackChance` | 指定（既定 `attack`） | `self` | 支払わない |
| `ninjaStrike` | 攻撃**または**ブロック（固定） | `opponent` | 支払わない |
| `revolutionChange` | 攻撃（固定） | `self` | 支払わない |

**1回の攻撃で条件を満たすものは、まとめて1つの一覧に並びます。**
たとえばカツキングで攻撃した時、ラフルル・ラブの革命チェンジと百鬼の邪王門の鬼エンドは
**同時に宣言でき**、解決の順番はリゾルバが1つずつ選ばせます（→ §7.28）。

> **革命チェンジは `revolutionChangeCond` に条件を書きます**（`keywords` に `revolutionChange` も必要）。
> 他の4つと違い「出す」のではなく攻撃クリーチャーとの**入れ替え**なので、専用の処理へ回ります。
> 入れ替わった後は、新しいクリーチャーの「攻撃する時」が誘発し、**続けて革命チェンジもできます**。
> 一方、鬼エンドなどは1回の攻撃につき一度なので、入れ替えの後には再提示しません。
