...
- 同じ攻撃に対して**複数枚まとめて宣言できます**。2枚以上宣言した場合は、そのあと解決順を選びます
  （1枚ぶんを解決しきってから次へ進みます）
...
>
> 実装では、唱えた時に `pendingSpellAfterCast` へ積んでおき、**リゾルバがアイドルになった時点**
> （＝その呪文を解決しきった時点）で盤面を見直して判定します。誘発した能力の解決よりも先に行います。
...
- 呪文を唱えられない状態（→ §7.23）なら、呪文の S・トリガーは提示されません。
  **クリーチャーの S・トリガーは通ります**
- 複数枚まとめてブレイクされた場合は、**まとめて宣言できます**。2枚以上宣言した場合は、
  そのあと解決順を選びます（1枚ぶんを解決しきってから次へ進みます）
...
- `replaceEnter`: 出る時の置換（超次元ゾーンへ送る） → **§7.28**
