...
// CUT-IN
// ===========================
// 自動で閉じるのを待たずに、クリック／タップですぐ飛ばせる。
// 何度押しても onDone は一度しか呼ばない（自動クローズとの二重発火を防ぐ）。
const CUTIN_MS = 1100;
...
export function CutIn({cutin,onDone}){
  ...
  useEffect(()=>{
    setVisible(true);
    const t=setTimeout(finish,CUTIN_MS);
    return()=>clearTimeout(t);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[]);
  const c=CIV[cutin.civ]||CIV.fire;
  return(
    <div onClick={finish} style={{position:"fixed",inset:0,zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",background:visible?`radial-gradient(ellipse at center,${c.glow}22 0%,transparent 70%)`:"transparent",transition:"background 0.3s"}}>
      <div style={{transform:visible?"scale(1) translateY(0)":"scale(0.6) translateY(30px)",opacity:visible?1:0,transition:"all 0.25s cubic-bezier(0.34,1.56,0.64,1)",display:"flex",flexDirection:"column",alignItems:"center",background:`linear-gradient(135deg,${c.bg}ee,#08080fee)`,border:`3px solid ${c.color}`,borderRadius:16,padding:"18px 40px",boxShadow:`0 0 40px ${c.glow}88,0 0 80px ${c.glow}44`,minWidth:240}}>
        ...
        <div style={{marginTop:10,fontSize:9,color:"#888"}}>クリック／タップでスキップ</div>
      </div>
    </div>
  );
}
