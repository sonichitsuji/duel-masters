...
// modal: { title, message, card?, applyLabel?, cancelLabel?, options? }
//
// options を渡すと「同じイベントを置換する効果が複数ある」時の選択になる。
// DMでは**1つだけ選んでそれだけを適用**し、選ばなかったものは起きない。
// onApply(i) に選んだ番号が渡る（options 無しの時は 0）。
// ===========================
export function ReplacementModal({ modal, onApply, onCancel }) {
  if (!modal) return null;
  const { title, message, card, applyLabel, cancelLabel, options } = modal;
  return (
      ...
      <div style={{ background:"linear-gradient(160deg,#0a1020,#08080f)", border:"2px solid #66ddff", borderRadius:14, padding:20, maxWidth:400, width:"100%", boxShadow:"0 0 30px #66ddff55" }}>
        ...
        {options ? (
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {options.map((o, i) => (
              <button key={i} onClick={() => onApply(i)} style={{ padding:"12px 14px", borderRadius:6, fontWeight:700, fontSize:12, background:"linear-gradient(135deg,#66ddff55,#66ddff22)", border:"1px solid #66ddff", color:"#66ddff", cursor:"pointer", textAlign:"left" }}>
                {o.label}
              </button>
            ))}
            <button onClick={onCancel} style={{ padding:"10px 14px", borderRadius:6, background:"#1a0a0a", border:"1px solid #aa5555", color:"#e88", cursor:"pointer", fontSize:12, fontWeight:700 }}>
              {cancelLabel || "どれも適用しない（例外処理）"}
            </button>
          </div>
        ) : (
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={() => onApply(0)} style={{ flex:1, padding:"10px", borderRadius:6, fontWeight:700, fontSize:12, background:"linear-gradient(135deg,#66ddff55,#66ddff22)", border:"1px solid #66ddff", color:"#66ddff", cursor:"pointer", fontFamily:"'Cinzel',serif" }}>
              {applyLabel || "置換を適用"}
            </button>
            <button onClick={onCancel} style={{ padding:"10px 14px", borderRadius:6, background:"#1a0a0a", border:"1px solid #aa5555", color:"#e88", cursor:"pointer", fontSize:12, fontWeight:700 }}>
              {cancelLabel || "例外処理で中止"}
        ...
        <div style={{ fontSize:9, color:"#555", marginTop:8, textAlign:"center" }}>
          {options ? "1つだけ選んで適用します。選ばなかった置換は起きません" : "「例外処理で中止」を選ぶと置換は行われず、元の処理のまま進みます"}
        </div>
      </div>
    </div>
  );
