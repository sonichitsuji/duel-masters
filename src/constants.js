...
// ===========================
// 能力テキストの中で強調表示するキーワード。行頭に限らず、文中に出てきたものも光らせる。
// 「進化」のように普通の文にも出る短い語は、誤爆するので入れない。
export const KEYWORD_PATTERNS = [
  "スピードアタッカー", "ワールド・ブレイカー", "W・ブレイカー", "T・ブレイカー", "ブロッカー",
  "S・トリガー", "G・ストライク", "ハイパーモード", "ハイパー化", "エスケープ", "スレイヤー",
  "マッハファイター", "ガードマン", "パワーアタッカー", "シールド・セイバー", "チャージャー",
  "革命チェンジ", "ファイナル革命", "メガ・ラスト・バースト", "ニンジャ・ストライク",
  "S・バック", "G・ゼロ", "G城", "Zラッシュ", "メテオバーン", "超魂X",
  "シビルカウント", "マナ武装", "W・ソウル", "ヒストリック・フィールド", "鬼エンド",
  "ジャストダイバー", "D・D・D", "革命", "G-NEO進化", "NEO進化",
];

// 長いキーワードから先に当てる（「W・ブレイカー」を「ブレイカー」に食われないように）。
// 1つだけキャプチャを持たせているので、split の結果は奇数番目がキーワードになる。
export const KEYWORD_REGEX = new RegExp(
  `(${[...KEYWORD_PATTERNS].sort((a, b) => b.length - a.length)
